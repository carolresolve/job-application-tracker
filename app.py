...
def index():
    ...
    total_candidatos = banco["candidatos"].count_documents({})
    total_empresas = banco["empresas"].count_documents({})
    total_candidaturas = banco["candidaturas"].count_documents({})

    # 2. Contagem por Status para o Gráfico
    status_counts = {
        "Em Análise": banco["candidaturas"].count_documents({"status": "Em Análise"}),
        "Entrevista Agendada": banco["candidaturas"].count_documents({"status": "Entrevista Agendada"}),
        "Em Teste Técnico": banco["candidaturas"].count_documents({"status": "Em Teste Técnico"}),
        "Aprovado": banco["candidaturas"].count_documents({"status": "Aprovado"}),
        "Rejeitado": banco["candidaturas"].count_documents({"status": "Rejeitado"}),
    }

    # 3. Taxa de Aprovação
    aprovados = status_counts.get("Aprovado", 0)
    taxa_aprovacao = round((aprovados / total_candidaturas) * 100, 1) if total_candidaturas > 0 else 0.0
